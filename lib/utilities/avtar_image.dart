...
class AvatarImage extends StatelessWidget {
  const AvatarImage(this.name, {this.width = 100, this.height = 100, this.bgColor, this.borderWidth = 0, required this.borderColor, this.trBackground = false, this.isSVG = true, this.radius = 50});
  final String name;
  final double width;
  final double height;
  ...
  Widget build(BuildContext context) {
    ...
    Container(width: width, height: height,
      ...
      child: SvgPicture.string(multiavatar(name, trBackground: trBackground)),
        ...
        :
    Container(width: width, height: height,padding: EdgeInsets.all(25),
      decoration: BoxDecoration(
        border: Border.all(color: borderColor ?? Theme.of(context).cardColor, width: borderWidth),
        color: bgColor,
